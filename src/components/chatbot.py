...
def _build_book_context(similar_books: list[dict]) -> str:
    ...
    for row in similar_books:
        ...
        lines.append(
            ...
            f"가격: {price_str}{sale_index_str} | 링크: {link}"
        )
    return "\n".join(lines)
    ...
    try:
        ...
        if is_numeric_query:
            ...
            with st.spinner("🔍 관련 도서를 벡터 공간에서 검색하고 있습니다..."):
                # 💡 불용어(Stopwords) 전처리를 수행하여 의미 검색의 노이즈 제거 ("도서", "추천" 등 제거)
                clean_query = user_input
                for word in ["도서", "서적", "추천", "알려줘", "해줘", "보여줘", "있어", "관련", "질문", "해달라고", "보여", "책"]:
                    clean_query = clean_query.replace(word, "")
                clean_query = clean_query.strip()
                if not clean_query:
                    clean_query = user_input
                similar_books = search_similar_books(clean_query, top_n=5)

        # 💡 [안전 장치 1] 유사도가 지나치게 낮은 도서 (20% 미만)는 의미적으로 무관한 도서로 보고 목록에서 자동 제외
        if similar_books:
            similar_books = [book for book in similar_books if book.get("similarity", 1.0) >= 0.20]

        # 💡 [안전 장치 2 (Fallback)] 벡터 검색이나 필터링 결과가 아예 0건인 경우
